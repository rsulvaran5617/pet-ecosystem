alter table public.pet_adoption_listings
  add column if not exists responsibility_acknowledged_at timestamptz,
  add column if not exists responsibility_acknowledged_by_user_id uuid references public.profiles(id);
