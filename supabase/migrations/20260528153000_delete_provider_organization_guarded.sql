...
begin
  ...
  using public.provider_documents as document
  where document.organization_id = target_organization.id
    and object.bucket_id = document.storage_bucket
    and object.name = document.storage_path;

  delete from storage.objects as object
  using public.provider_public_profiles as profile
  where profile.organization_id = target_organization.id
    and profile.avatar_storage_bucket is not null
    and profile.avatar_storage_path is not null
    and object.bucket_id = profile.avatar_storage_bucket
    and object.name = profile.avatar_storage_path;
