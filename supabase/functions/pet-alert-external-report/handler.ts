...
import { PET_ALERT_PHOTO_MAX_BYTES, PetAlertPhotoError, sanitizePetAlertPhoto } from "../_shared/pet-alert-media.ts";

const TERMS_VERSION = "pet-alert-external-v1";
const PRIVACY_VERSION = "pet-alert-external-v1";
const MAX_FILE_BYTES = PET_ALERT_PHOTO_MAX_BYTES;
...
async function submitReport(request: Request, origin: string) {
  ...
  try {
    ...
    if (error instanceof PetAlertPhotoError) {
      return json(origin, { ok: false, message: "Una foto no pudo procesarse. Usa JPG, PNG o WebP sin animacion, de hasta 5 MB y 12 megapixeles. Revisa las fotos y solicita un nuevo codigo para reintentar." }, 400);
    }
    throw error;
  }
...
export async function handleExternalPetAlertRequest(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (contentType.includes("application/json")) return await requestCode(await boundedRequest(request, 64 * 1024), origin);
    if (contentType.includes("multipart/form-data")) return await submitReport(await boundedRequest(request, 21 * 1024 * 1024), origin);
    return json(origin, { ok: false, message: "Solicitud no valida." }, 415);
  } catch (error) {
    if (error instanceof RequestTooLargeError) return json(origin, { ok: false, message: "La solicitud supera el tamano permitido." }, 413);
    console.error("pet-alert-external-report failed", error instanceof PetAlertPhotoError ? "PET_ALERT_PHOTO_INVALID" : "PET_ALERT_REQUEST_FAILED");
