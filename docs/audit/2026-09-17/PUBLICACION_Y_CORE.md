...
- Estado observado: **IN_QUEUE**, creado a las 13:39:10 UTC del 18/09/2026. No hay APK descargado, instalación ni distribución confirmadas.
...
2. Consultar `eas build:view 795bad45-4d7d-48c1-8d4f-2b889b5071df` desde `apps/mobile` de la copia aislada. Si termina, descargar APK, registrar hash, instalar en dispositivo y probar revocación H05, sesión/roles y permisos de documentos. Si falla, revisar logs antes de reintentar.
...
`evidence/deployed-public-check.json`, `evidence/core-read-isolation.json` y `evidence/release-followup.json` guardan resultados sin datos personales ni secretos.
