...
`pet_profiles.foster_intake_date` hereda las mismas reglas de lectura/escritura del perfil de mascota: solo miembros con permiso `edit` o `admin` pueden cambiarlo via `create_pet`/`update_pet`; no agrega policy nueva ni exposicion publica.
...
FOSTER-MEDIA-DIRECT-1 permite agregar fotos a publicaciones `published` sin cambiar el estado de la publicacion: la media nueva de una Familia Protectora aprobada queda `approved` bajo responsabilidad directa, y adoptantes solo leen media `approved`. Admin conserva moderacion posterior mediante RPC y puede retirar/rechazar fotos reportadas. La Familia Protectora puede retirar fotos de su propia publicacion; no actualiza directamente `moderation_status`. El limite inicial es 8 fotos por publicacion y solo una portada puede quedar marcada.
...
- Foster-5A public profile: el perfil publico de familia protectora debe vivir separado del perfil interno de aprobacion; lectura publica/autenticada solo para `approved` + `is_public`. Una familia aprobada puede corregir un perfil publico ya aprobado, pero el cambio vuelve el perfil a `draft`, marca `is_public = false` y exige reenviar a revision antes de volver a mostrarse publicamente.
- Foster public social links: los enlaces de redes viven en `protective_household_public_profiles`, heredan las mismas policies del perfil publico y solo deben leerse publicamente cuando el perfil esta `approved` + `is_public`.
- Foster public donations/support: la informacion opcional de apoyo vive en `protective_household_public_profiles`, hereda las mismas policies y moderacion del perfil publico, y solo se expone si el perfil esta `approved` + `is_public`, `donations_enabled = true` y existe contenido declarado. No habilita pagos, checkout ni obligacion para adoptar; admin revisa esta informacion como parte de la aprobacion del perfil publico.
...
- Foster publicacion responsable directa: `submit_pet_adoption_listing(..., responsibility_acknowledged)` exige familia protectora aprobada, household `protective`, permiso de gestion sobre el listing y aceptacion explicita de responsabilidad. Admin conserva lectura/auditoria y puede pausar o rechazar contenido posteriormente; publicar no mueve custodia ni crea solicitudes.
- Foster adoption listing management scope 2026-08-16: el helper `can_manage_pet_adoption_listing` acepta dos scopes seguros para evitar bloqueos operativos en fichas previas o con custodia vigente: admin del household registrado en `pet_adoption_listings.household_id` o admin del household protector que actualmente custodia `pets.household_id`. En ambos casos el household debe ser `protective` aprobado. No concede gestion a owners regulares, providers comerciales, adoptantes ni anonimos.
- Foster-6A/6B compromiso documental: `foster-adoption-documents` es bucket privado. La familia protectora aprobada gestiona solo su plantilla activa; el solicitante autenticado ve la plantilla de su solicitud y sube/reemplaza solo su documento firmado; la familia protectora propietaria de la publicacion puede leerlo y marcarlo `reviewed` o `needs_correction`; admin conserva lectura de auditoria. No hay lectura publica anonima de documentos.
- FOSTER-EXPENSES-1: `foster_pet_expenses` es privado del household `protective`; miembros con permiso de vista sobre la mascota leen, miembros con permiso de edicion/admin crean/editan/eliminan, `receipt_document_id` debe pertenecer a la misma mascota y no hay lectura publica ni provider comercial.
...
ambos, aplicada remoto el 2026-09-20: revoca PUBLIC/anon y exige condicion
...
ACL y guard con JWT real/UUID inexistente verificados tras aplicar; falta regresion
completa de RLS/Storage entre hogares antes del cierre de Foundation.
