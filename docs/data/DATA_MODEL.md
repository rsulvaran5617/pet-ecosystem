...
- FOSTER-DONATIONS-1 extiende `protective_household_public_profiles` con informacion opcional de apoyo declarada por la Familia Protectora: `donations_enabled`, `donation_title`, `donation_description`, `donation_ach_details`, `donation_yappy_details`, `donation_paypal_details`, `donation_external_url`, `donation_other_details` y `donation_disclaimer`. Estos campos son contenido publico moderado, no pagos, no checkout y no condicionan solicitudes de adopcion.
...
- Foster publicacion responsable directa agrega `responsibility_acknowledged_at` y `responsibility_acknowledged_by_user_id` a `pet_adoption_listings`; una Familia Protectora aprobada puede publicar/actualizar ficha publica sin revision previa de cada listing, manteniendo auditoria y pausa posterior por plataforma.
...
- Foster-6A/6B agrega compromiso documental de adopcion sin firma digital: `protective_household_adoption_commitment_templates` guarda una plantilla privada activa por Familia Protectora y `pet_adoption_application_commitment_documents` guarda el documento completado por solicitud con estado `pending`, `received`, `reviewed` o `needs_correction`.
