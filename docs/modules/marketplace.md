...
- Foster-4A agrega entrada `Adopcion` / `Mascotas que buscan hogar` dentro de Owner mobile Buscar, separada del marketplace comercial de servicios. Solo lista publicaciones Foster aprobadas; no crea reservas, solicitudes ni transferencias. Owner Inicio puede enlazar a esta ruta con un CTA compacto; Owner Mascotas queda enfocado en gestion propia y publicaciones propias de familias protectoras.
...
- Owner Buscar incluye discovery Foster separado: cards de mascotas publicadas con foto, nombre, especie, edad, ciudad/pais, esterilizacion, resumen publico y detalle read-only. El CTA `Me interesa` solo muestra aviso informativo durante piloto. Esta es la ruta canonica de discovery de adopcion para evitar duplicar una vitrina pesada dentro de `Mascotas`.
