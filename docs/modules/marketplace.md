...
- Owner Buscar > proveedor conserva solo una identificacion compacta del hogar/familia activo antes del detalle del proveedor; no muestra bloques internos de progreso como `Explorar`, `Resultados` o `Proveedor`, ni badges de origen como `desde resultados`.
- UX-MOBILE-FLOWS Slice 1M simplifica el paso proveedor/servicio: cada servicio abre sus horarios y cupos como acordeon inline con CTA `Ver horarios` / `Ocultar horarios`; se elimina la pantalla separada de disponibilidad para reducir clicks. `Continuar reserva` transfiere proveedor, servicio, hogar/mascota y slot elegido a Reservas sin crear reserva ni consumir cupo desde Buscar.
