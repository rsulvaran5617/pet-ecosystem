...
- Foster Web Console: `/foster` es una consola web separada para familias protectoras. Debe mostrar estados sin sesion/sin familia protectora/en revision/aprobada; si no existe familia protectora, ofrece CTA `Crear Familia Protectora` con formulario compacto y envio a revision admin. Para familias aprobadas muestra selector de familia, bloque `Perfil publico` con formulario compacto/estado/envio a revision, dashboard de solicitudes/publicaciones/transferencias, inbox de solicitudes, detalle con timeline y CTAs contextuales. No debe mezclar datos de provider web ni admin web.
