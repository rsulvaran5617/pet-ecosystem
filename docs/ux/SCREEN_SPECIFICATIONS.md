...
- Foster-5D familia protectora: `Solicitudes recibidas` debe funcionar como bandeja/pipeline por mascota con resumen de nuevas/en revision/entrevista/aprobadas/rechazadas, filtros por mascota/publicacion/estado/fecha, detalle estructurado del solicitante, timeline de estado y CTAs contextuales. Foster-5D.1 ya deja preparada la base de detalle/historial/cambio de estado; la UI completa queda para Foster-5D.2. El copy debe aclarar que `Aprobar solicitud no transfiere la mascota`; iniciar transferencia queda para Foster-5E.
