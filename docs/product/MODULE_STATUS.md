...
- Foster/Adoption V2.5 queda en `documented_on_hold`: `docs/modules/foster_adoption.md` define hogares de acogida, rescate y adopcion como nodo no financiero para mascotas bajo custodia temporal, publicaciones `Busca hogar`, solicitudes, moderacion y transferencia de custodia digital. No es provider comercial, no habilita pagos ni venta de mascotas y requiere aprobacion especial antes de publicar.
...
- iOS Beta Privada queda preparada documental/configurativamente para EAS + TestFlight: `apps/mobile/app.json` define version `0.3.1`, `ios.buildNumber` inicial, bundle id `com.petecosystem.mobile`, scheme `petecosystem` y textos de permisos iOS; `apps/mobile/eas.json` explicita build iOS no-simulator en preview/production. No se genero build iOS ni se subio a TestFlight todavia.
