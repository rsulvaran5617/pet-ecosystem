...
- Role-Foster-A queda preparado localmente: se agrega rol global `protective_family` para diferenciar Familias Protectoras desde registro/cambio de modo, manteniendo permisos Foster reales en `household_type = protective` y perfil protector aprobado. Requiere aplicar migracion `20260725103000_core_protective_family_role.sql`; no hay backfill ni conversion de hogares owner.
