...
- Foster-5A queda implementado localmente y pendiente de aplicar remoto: perfil publico moderado de Familia Protectora con `protective_household_public_profiles`, RLS, RPCs tipados, owner mobile en Hogares y cola admin de revision. Foster-5B queda implementado localmente y pendiente de aplicar remoto: ficha publica de mascota por slug con lectura publica controlada, galeria aprobada y vista web `/adopciones/[slug]`. Foster-5C queda implementado localmente y pendiente de aplicar remoto: `pet_adoption_applications`, solicitud autenticada desde mobile, retiro de solicitud y auditoria admin basica. Foster-5D.1 queda implementado localmente: historial auditable, estado `interview`, RPCs de detalle/cambio de estado/historial y API client tipado; la UI completa de bandeja queda para Foster-5D.2. Foster-5E queda implementado localmente y pendiente de aplicar remoto: solicitud `approved` puede iniciar transferencia privada vinculada, aceptar la transferencia mueve custodia por Foster-2A, marca la solicitud `converted_to_transfer` y cierra la publicacion como `adopted`.
