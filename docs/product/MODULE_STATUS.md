...
- `apps/web` queda depurada por rol activo: usuarios duales ven solo la consola provider cuando el rol activo es `provider`, ven owner cuando el rol activo es `pet_owner` y el rol `protective_family` deriva a `/foster`.
...
- Foster publicacion responsable directa queda preparada localmente: Web `/foster` muestra/edita la ficha publica de adopcion con historia, personalidad, salud, compatibilidad, requisitos y ubicacion; `submit_pet_adoption_listing(..., responsibility_acknowledged)` publicara directo bajo responsabilidad de la Familia Protectora aprobada y admin queda como moderador posterior. Pendiente aplicar migracion remoto `20260726123000_foster_responsible_direct_publication.sql`.
- Foster logo de perfil publico queda implementado localmente: agrega bucket privado `protective-household-logos`, metadata `logo_storage_bucket/path`, API/UI web+mobile y moderacion obligatoria al cambiar logo. No aplicar remoto sin dry-run/aprobacion.
- Foster-6A/6B compromiso documental queda implementado y aplicado remoto: plantilla privada de compromiso por Familia Protectora, documento firmado por solicitud, API/tipos, Web Foster para carga/revision y Owner mobile adopcion para descarga/subida.
- Foster-7A/7B/7C queda implementado sin migraciones: rechazo humano con mensaje editable, checklist `Cierre responsable` en solicitudes aprobadas/transferencias y trazabilidad de cierre adoptivo. Si el compromiso esta configurado como requerido antes de transferir, la UI espera revision documental antes de iniciar transferencia.
- Web Foster `Publicaciones` agrega historia read-only de adopciones cerradas/adoptadas con resumen de adoptante, solicitud, transferencia y eventos en acordeon, reutilizando datos existentes sin nuevas migraciones.
- Foster redes sociales publicas queda aplicado remoto: `protective_household_public_profiles` suma enlaces opcionales de sitio web/Instagram/Facebook/TikTok/WhatsApp, Web Foster permite editarlos desde Perfil y Owner mobile los muestra solo dentro de `Ver perfil` de mascotas en adopcion. Migracion `20260726170000_foster_protective_public_social_links.sql` aplicada con dry-run posterior limpio.
- Adoption Public Funnel Slice 4 queda implementado y aplicado remoto: formulario publico ligero por mascota, controles antiabuso, trazabilidad y bandeja Web Foster de interes publico separada de solicitudes formales; no crea transferencias ni cambia custodia.
- Adoption Public Funnel Slice 5 queda implementado y aplicado remoto: estado `invited_to_app`, invitacion temporal con token hasheado, CTA de generacion/copia en Web Foster y pagina puente publica con deep link.
- Adoption Public Funnel Slice 6 queda implementado y aplicado remoto: claim autenticado por correo, continuidad mobile desde deep link, hogar Owner obligatorio y conversion idempotente a solicitud formal; no mueve custodia ni crea transferencia.
...
- Store-DEL-1 implementa la solicitud de eliminacion de cuenta en mobile: `request_account_deletion()` anonimiza perfil/direcciones, desactiva roles y metodos guardados, bloquea acceso futuro en Auth y conserva historial transaccional para soporte/auditoria. Requiere aplicar migracion `20260727120000_core_account_deletion_request.sql`; no ejecuta borrado fisico de reservas, chats, soporte ni resenas.
...
- PET ALERT tiene Slice 1A, Slice 4 comunitario, Slice 5, Slice 6, Slice 7A y Slice 7B aplicados remoto, ademas de Slice 2 mobile owner y Slice 3 de ficha publica. Slice 7B incorpora el centro comunitario publico unificado con busqueda, filtros y paginacion. Escritura anonima y notificaciones push siguen diferidas.
- PET ALERT 8B queda versionado y con migracion aplicada remoto: propietario sin cuenta, Turnstile, OTP por Resend, 1-4 fotos, boletin canonico `pending_review` y cola admin. No crea usuario Auth ni publica sin moderacion. Para operar el formulario aun requiere configurar secretos y desplegar la Edge Function. La moderacion automatica de imagenes queda documentada como propuesta futura pendiente de aprobacion; la revision admin sigue siendo obligatoria.
...
- Clinical Access-2B: identidad profesional y verificacion Admin implementadas localmente en modo solo lectura; migracion remota y QA pendientes.
