...
- iOS Beta Privada queda preparada documental/configurativamente para EAS + TestFlight: `apps/mobile/app.json` define version `0.3.1`, `ios.buildNumber` inicial, bundle id `com.petecosyst.mobile`, scheme `petecosystem` y textos de permisos iOS; `apps/mobile/eas.json` explicita build iOS no-simulator en preview/production. No se genero build iOS ni se subio a TestFlight todavia.
