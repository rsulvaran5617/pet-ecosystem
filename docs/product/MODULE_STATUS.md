...
- Foster-3B queda implementado, versionado y publicado en Git (`d18325a`), pendiente de aplicar remoto: migracion `20260628120000_foster_3b_adoption_media_gallery.sql`, RLS para agregar fotos a publicaciones `published` sin despublicarlas, limite de 8 fotos, portada controlada, moderacion individual admin y filtros publicos para mostrar solo media aprobada. No incluye videos, chat ni solicitudes formales de adopcion. APK Android QA generada en `dist/pilot/android/pet-ecosystem-pilot-v0.3.1-foster3b-android.apk`; iOS Foster-3B queda bloqueado temporalmente por limite mensual EAS iOS hasta el reinicio del plan.
...
- Foster household separation queda documentado como siguiente frente de estabilizacion: hogares `owner` y `protective` deben separarse mediante tipo operativo de household, manteniendo usuarios multi-hogar cuando necesiten ambos contextos. No hay migracion ni codigo funcional todavia; el siguiente slice debe agregar `household_type`, validaciones RLS/RPC y UX diferenciada sin duplicar mascotas ni borrar historial.
- Foster-Household-B queda preparado localmente: reporte `docs/delivery/FOSTER_HOUSEHOLD_TYPE_IMPACT_REPORT.md`, migracion `20260629110000_household_type_owner_protective.sql`, tipos/API para `householdType`, creacion explicita de hogares `owner`/`protective`, UX owner en Hogares para distinguir `Hogar familiar` vs `Familia protectora` y validaciones server-side Foster. Decision de producto: `HOGAR SULVARAN VELASCO` permanece como hogar familiar `owner`; Foster debe operar luego desde un household `protective` separado. No aplicar remoto sin preparar ese household protector o aceptar que Foster quede bloqueado temporalmente para el hogar owner.
