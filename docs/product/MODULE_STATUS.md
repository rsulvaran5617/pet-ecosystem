...
- Foster-4A queda implementado en mobile owner Buscar como discovery controlado: entrada `Adopcion`, listado `Mascotas que buscan hogar`, detalle publico con galeria/historia/salud/compatibilidad/requisitos y CTA informativo. `Inicio` puede llevar a esta ruta con CTA compacto y `Mascotas` queda enfocado en gestion propia/publicaciones propias de familias protectoras. Commit publicado: `d643357`. No crea solicitudes, transferencias, reservas ni chats; usa APIs Foster-3A existentes y no requiere migracion.
