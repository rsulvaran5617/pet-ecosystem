...
- Production data cleanup provider queda documentado y aplicado de forma no destructiva: `docs/delivery/PRODUCTION_DATA_CLEANUP_RUNBOOK.md`, `PRODUCTION_DATA_CLEANUP_REVIEW.sql`, `PRODUCTION_DATA_CLEANUP_CANDIDATES.sql` y `PRODUCTION_DATA_CLEANUP_APPROVED_HIDE.sql` definen diagnostico, clasificacion, ocultamiento no destructivo y borrado solo para organizaciones sin historia transaccional mediante RPC guardada. Marketplace conserva visibles `AMERICAN PETS`, `American Pets local Sede` y `BLACK DOG BRISAS`; los demas providers actuales fueron ocultados/desactivados sin borrar historial transaccional.
...
- Foster-5A a Foster-5E quedan aplicados remoto y publicados: perfil publico moderado de Familia Protectora, ficha publica de mascota por slug, solicitudes estructuradas de adopcion, historial/cambio controlado de estado y cierre `adopted` conectado a transferencia privada Foster-2A. La regla clave se mantiene: aprobar una solicitud no mueve custodia; solo aceptar la transferencia privada mueve `pets.household_id`, marca la solicitud `converted_to_transfer` y cierra la publicacion como `adopted`. Hotfixes remotos `20260707173000_foster_adoption_detail_ambiguous_id_fix.sql` y `20260707184500_foster_transfer_invitations_ambiguous_id_fix.sql` corrigen el detalle de publicaciones y la carga de invitaciones de transferencia para evitar `column reference "id" is ambiguous` en mobile `Mascotas que buscan hogar`. Foster-5D.2 queda implementado localmente en mobile como bandeja/pipeline para familias protectoras aprobadas, con metrica/aviso de aprobadas pendientes de transferencia y CTA explicito para iniciar transferencia. Foster Web Console queda implementada localmente en `/foster` como consola separada para familias protectoras: permite crear/solicitar Familia Protectora desde web cuando no existe; para familias aprobadas permite crear/editar/enviar a revision el perfil publico, registrar/listar mascotas bajo acogida, guiar cada publicacion con tren `Mascota -> Publicacion -> Contenido -> Fotos -> Revision -> Visible`, gestionar galeria publica moderada de Foster-3B y destacar solicitudes aprobadas pendientes de transferencia; pendiente QA visual antes de cierre Git.
