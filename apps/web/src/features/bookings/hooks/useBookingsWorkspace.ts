...
export function useBookingsWorkspace(
  ...
  async function loadBookings(householdId: Uuid | null, petId: Uuid | null) {
    ...
    const nextBookings = await getBrowserBookingsApiClient().listBookings({
      householdId,
      petId,
  ...
  async function refresh(selectionOverride?: MarketplaceServiceSelection | null) {
    ...
    try {
      const [nextHouseholdSnapshot, nextCoreSnapshot] = await Promise.all([
        getBrowserHouseholdsApiClient().getHouseholdsSnapshot(),
        getBrowserCoreApiClient().getCoreSnapshot()
      ]);
  ...
  return {
    ...
    async cancelBooking(bookingId, reason) {
      await runAction(async () => {
        const detail = await getBrowserBookingsApiClient().cancelBooking(bookingId, reason);
