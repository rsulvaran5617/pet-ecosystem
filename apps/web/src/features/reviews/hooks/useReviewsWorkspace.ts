...
import { getBrowserReviewsApiClient } from "../../core/services/supabase-browser";
...
export function useReviewsWorkspace(
  ...
  async function loadReviewState(bookingId: Uuid) {
    const nextState = await getBrowserReviewsApiClient().getBookingReviewState(bookingId);
  ...
  return {
    ...
    async submitReview() {
      ...
      await runAction(
        () =>
          getBrowserReviewsApiClient().createReview({
            bookingId,
            rating: ratingDraft,
