...
export function useMarketplaceWorkspace(enabled: boolean): UseMarketplaceWorkspaceResult {
  ...
  async function refresh() {
    ...
    try {
      const [nextHouseholdSnapshot, nextHomeSnapshot] = await Promise.all([
        getMobileHouseholdsApiClient().getHouseholdsSnapshot(),
        getMobileMarketplaceApiClient().getMarketplaceHome()
      ]);
  ...
  return {
    ...
    async search(filters) {
      ...
      try {
        const nextProviders = await getMobileMarketplaceApiClient().listMarketplaceProviders(filters);
    ...
    async openProvider(providerId) {
      ...
      try {
        const providerDetail = await getMobileMarketplaceApiClient().getMarketplaceProvider(providerId);
